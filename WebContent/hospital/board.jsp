...
<body>
  ...
  <div class="board">
      <div class="board2">
          <div>
              <div>
                  <h1 class="gongzi">QnA 게시판</h1>
              </div>
              <table class="table_3">
                  <!-- 콜그룹으로 열의 비율을 맞춤 두번째것은 값을 안넣었으니 자동으로 남은 비율을 가진다-->
                  <colgroup>
                  ...
                  <tbody>
                      ...
                      <c:if test="${count != 0}">
                          ...
                          <c:forEach var="bto" items="${btos}">
                              <tr>
                                  <td style="text-align: center">${number}
                                      <c:set var="number" value="${number-1}" />
                                  </td>
                                  <td style="text-align:;">
                                      <c:set var="wid" value="${(bto.re_level-1) * 10}" />
                                      <c:if test="${bto.re_level gt 1}">
                                          <img src="./img/level.gif" width="${wid}" height="15">
                                      </c:if>
                                      <c:if test="${bto.re_level gt 0}">
                                          <img src="./img/re.gif" width="15" height="15">
                                      </c:if>
                                      <c:if test="${bto.readcount == -1}">
                                          ${bto.b_title}
                                      </c:if>
                                      <c:if test="${bto.readcount != -1}">
                                          <a href="boardcontent.do?b_no=${bto.b_no}&pageNum=${pageNum}&number=${number+1}">
                                              ${bto.b_title}
                                          </a>
                                      </c:if>
                                  </td>
                                  <!-- 여기가 제목부분 디자인 먹는곳 개가튼놈 -->
                                  <td style="text-align: center">${bto.fk_user_id1}
                                      ${bto.fk_dr_id1}</td>
                                  <td style="text-align: center">
                                      <fmt:formatDate value="${bto.b_reg_date}" pattern="yyyy-MM-dd HH:mm" />
                                  </td>
                                  <td style="text-align: center">
                                      <c:if test="${bto.readcount == -1}">
                                          &nbsp;
              ...
              <div class="">
                  <span class="glejacksung"> <a href="boardinput.do">
                  <h3 class="glejacksung2">글작성</h3></a></span>
              </div>
              <div class="table_lr">
                  <c:if test="${count gt 0}">
                      <c:if test="${startPage gt pageBlock}">
                          <a href="board.do?pageNum=${startPage - pageBlock}">[◀]</a>
                      </c:if>
            ...
            <div class="footer_site">
                <div class="footer_top_site">
                    <select name="family_link" onchange="if(this.value) location.href=(this.value);">
                        <option value="https://www.snubh.org/index.do">분당서울대학병원</option>
                        <option value="http://www.snuh.org/intro.do">서울대학교병원</option>
                        <option value="https://healthcare.snuh.org/home.do">서울대학교병원강남센터</option>
                        <option value="https://www.cmcseoul.or.kr/page/main">가톨릭대학교성모병원</option>
                        <option value="https://www.brmh.org/main/main.do">서울 보라매병원</option>
                    </select>
                </div>
            </div>
                          ...
                          <c:forEach var="bto" items="${btos}">
                              <tr>
                                  ...
                                  <td>
                                      <c:set var="wid" value="${(bto.re_level-1) * 10}" />
                                      <c:if test="${bto.re_level gt 1}">
                                          <img src="./img/level.gif" width="${wid}" height="15"></c:if>
                                      <c:if test="${bto.re_level gt 0}">
                                          <img src="./img/re.gif" width="15" height="15"></c:if>
                                      <c:if test="${bto.readcount == -1}">
                                          ${bto.b_title}
                                      </c:if>
                                      <c:if test="${bto.readcount != -1}">
                                  ...
                                  <td style="text-align: center">${bto.fk_user_id1} ${bto.fk_dr_id1}</td>
                                  <td style="text-align: center">
                                      <fmt:formatDate value="${bto.b_reg_date}" pattern="yyyy-MM-dd HH:mm" />
                                  </td>
                                  <td style="text-align: center">
            ...
            <div class="footer_site">
                <div class="footer_top_site">
                    <select name="family_link" onchange="if(this.value) location.href=(this.value);">
                        ...
                        <option value="https://healthcare.snuh.org/home.do">서울대학교병원강남센터</option>
                        <option value="https://www.cmcseoul.or.kr/page/main">가톨릭대학교성모병원</option>
                        <option value="https://www.brmh.org/main/main.do">서울보라매병원</option>
                    </select>
                </div>
            </div>
            <div class="footer_bottom">
